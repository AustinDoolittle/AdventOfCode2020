...
public:
    ...
    size_t get_num_bags(const string& starting_node) const {
        size_t running_sum = 1;
        if (!adjacency_list.count(starting_node) || adjacency_list.at(starting_node).empty()) {
            return running_sum;
        }

        for (const auto& edge : adjacency_list.at(starting_node)) {
            running_sum += edge.weight * get_num_bags(edge.destination);
        }
...
//    auto input_file_stream = open_debug(7);
    ...
    while (getline(input_file_stream, line)) {
        ...
        while (regex_search(inner_bags_str, inner_bag_match, inner_bag_regex)) {
            ...
            graph.add_edge(outer_bag, inner_bag, weight);

            inner_bags_str = inner_bag_match.suffix();
        }
    }

    const auto result = graph.get_num_bags("shiny gold");
    std::cout << "Done! You gotta buy " << result - 1 << " bags." << std::endl;
}
